...
<?php dashboard_page_start('Wallet', [
    ...
    'css' => '
      ...
      .reserved-card { border-top:4px solid #2563eb; }
      ...
      .reserved-action:hover { border-color:#2563eb; background:#f1f5ff; }
      ...
      .wallet-fold { background:#fff; border:1px solid rgba(16,24,40,.08); border-radius:12px; box-shadow:0 12px 30px rgba(16,24,40,.08); overflow:hidden; margin-top:18px; }
      .wallet-fold summary { display:flex; align-items:center; justify-content:space-between; gap:12px; cursor:pointer; list-style:none; padding:16px 20px; font-weight:900; color:#111827; }
      .wallet-fold summary::-webkit-details-marker { display:none; }
      .wallet-fold summary::after { content:"+"; width:28px; height:28px; border-radius:50%; display:grid; place-items:center; background:#e8f6ec; color:var(--green); font-weight:900; transition:transform .2s ease; }
      .wallet-fold[open] summary::after { content:"−"; transform:rotate(180deg); }
      .wallet-fold-head { display:flex; align-items:center; justify-content:space-between; gap:12px; flex:1; min-width:0; }
      .wallet-fold-head h2 { margin:0; font-size:1.05rem; color:#0F3D1B; }
      .wallet-fold-head small { color:var(--muted); font-weight:700; font-size:.82rem; }
      .wallet-fold-body { padding:0 20px 20px; }
      .wallet-fold-body table { margin:0; }
      .wallet-fold-body th, .wallet-fold-body td { padding:10px 8px; }
      .wallet-fold-count { display:inline-flex; align-items:center; justify-content:center; min-width:24px; height:24px; border-radius:999px; background:#e8f6ec; color:var(--green); font-size:.78rem; font-weight:900; padding:0 8px; margin-left:6px; }
      /* Quick stats bar */
      .wallet-stats {
        display:grid;
      ...
      .wallet-stat-info small { display:block; color:var(--muted); font-size:.78rem; font-weight:700; }
      .wallet-stat-info strong { display:block; font-size:1.3rem; color:var(--ink); }
      @media(max-width:820px){ .wallet-grid,.wallet-actions-grid { grid-template-columns:1fr; } .fund-form input, .fund-form button { width:100%; } .wallet-fold summary { padding:14px 16px; } .wallet-fold-body { padding:0 16px 16px; } }
    ',
]); ?>
...
<div class="wallet-stats">
  <div class="wallet-stat-card">
    <div class="wallet-stat-icon"><i class="fas fa-wallet"></i></div>
    <div class="wallet-stat-info">
      <small>Available Balance</small>
      <strong>NGN <?= e(number_format((float) $wallet['balance'], 2)) ?></strong>
    </div>
  </div>
  <div class="wallet-stat-card">
    <div class="wallet-stat-icon" style="background:#fff4dc;color:var(--gold);"><i class="fas fa-hourglass-half"></i></div>
    <div class="wallet-stat-info">
      <small>Held for Withdrawals</small>
      <strong>NGN <?= e(number_format((float) ($wallet['hold_balance'] ?? 0), 2)) ?></strong>
    </div>
  </div>
  <div class="wallet-stat-card">
    <div class="wallet-stat-icon" style="background:#eaf1ff;color:var(--blue);"><i class="fas fa-exchange-alt"></i></div>
    <div class="wallet-stat-info">
...
<!-- Row 1: Wallet Balance + Reserved Transfer Account -->
...
<!-- Row 2: Fund Wallet + Withdraw Earnings -->
...
<details class="wallet-fold" <?= $withdrawals ? 'open' : '' ?>>
    <summary>
        <div class="wallet-fold-head">
            <div>
                <h2>Withdrawal Requests <span class="wallet-fold-count"><?= count($withdrawals) ?></span></h2>
                <small>Track payout requests and admin review status.</small>
            </div>
        </div>
    </summary>
    <div class="wallet-fold-body">
        <table>
...
<details class="wallet-fold" open>
    <summary>
        <div class="wallet-fold-head">
            <div>
                <h2>Recent Transactions <span class="wallet-fold-count"><?= count($transactions) ?></span></h2>
                <small>Funding, withdrawals, and marketplace settlements.</small>
            </div>
        </div>
    </summary>
    <div class="wallet-fold-body">
        <table>
            ...
            <?php foreach ($transactions as $tx): ?>
                ...
                $isPendingMonnifyFunding = (string) ($tx['type'] ?? '') === 'funding' && (string) ($tx['provider'] ?? '') === 'monnify' && (string) ($tx['status'] ?? '') === 'pending';
                $isPendingWithdrawal = (string) ($tx['type'] ?? '') === 'withdrawal' && (string) ($tx['status'] ?? '') === 'pending';
                ?>
                <tr>
...
<?php if ($isPendingMonnifyFunding && $checkoutUrl !== ''): ?>
<a class="button" href="<?= e($checkoutUrl) ?>">Continue Payment</a>
<?php endif; ?>
<?php if ($isPendingMonnifyFunding): ?>
<form method="post">
<input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
<input type="hidden" name="reference" value="<?= e($reference) ?>">
<button type="submit" name="check_payment_status" class="button secondary">Check Funding Status</button>
</form>
<?php endif; ?>
<?php if ($isPendingWithdrawal): ?>
<span class="badge pending" style="padding:6px 10px;font-size:.78rem;">Awaiting Admin Approval</span>
<?php endif; ?>
</div>
</td>
                </tr>
