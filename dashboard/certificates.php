...
$growerCertificatePaidRequired = $applicationId > 0 && grower_certificate_access_required($pdo, $application);
$growerCertificateAmount = grower_certificate_fee_amount($pdo, $memberType);
$growerCertificateAccess = $applicationId > 0 ? grower_certificate_access_status($pdo, $userId, $applicationId) : ['paid' => false, 'payment' => null];
$growerCertificatePaid = (bool) $growerCertificateAccess['paid'];
    ...
    if (!verify_csrf($_POST['_csrf'] ?? null)) {
        ...
        try {
            ...
            if ($action === 'issue_grower_certificate') {
                ...
                redirect_to('certificates.php?message=' . urlencode('Grower certificate issued. Pay the access fee to view or download where required.'));
            }
            ...
            if ($action === 'pay_grower_certificate') {
                ...
                $certificate = generateCertificate($applicationId, $userId, $pdo);
                if ($growerCertificatePaidRequired && !$growerCertificatePaid) {
                    grower_certificate_pay_access($pdo, $userId, $applicationId, (int) ($certificate['id'] ?? 0), 'view_download');
                }
                redirect_to('certificates.php?message=' . urlencode('Certificate access fee paid. You can now view and download your grower certificate.'));
            }
        } catch (Throwable $e) {
...
<?php if (!empty($_GET['error'])): ?><div class="notice error"><?= e((string) $_GET['error']) ?></div><?php endif; ?>
    ...
    <article class="cert-panel">
      <div class="cert-feature">
        ...
        <div>
          ...
          <p>Issued after identity/farm verification. Viewing and downloading require the operator-managed certificate access or renewal fee where enabled.</p>
        </div>
      </div>
      <div class="cert-meta">
        <div><span>Eligibility</span><strong><?= $canIssueGrowerCertificate ? 'Verification complete' : 'Verification incomplete' ?></strong></div>
        <div><span>Member Type</span><strong><?= e(grower_member_type_label($memberType)) ?></strong></div><div><span>Access Fee</span><strong><?= $growerCertificatePaidRequired ? 'NGN ' . e(number_format($growerCertificateAmount, 2)) : 'Free' ?></strong></div>
        <div><span>Access Status</span><strong><?= $growerCertificatePaidRequired ? ($growerCertificatePaid ? 'Active' : 'Payment required') : 'Not required' ?></strong></div>
        ...
        <div class="cert-actions">
          <?php if ($growerCertificatePaidRequired && !$growerCertificatePaid): ?>
            <form method="post">
              ...
              <button type="submit" <?= $walletBalance >= $growerCertificateAmount ? '' : 'disabled' ?>>Pay Access Fee & Issue</button>
            </form>
    ...
    <div class="cert-library">
      <?php foreach ($growerCertificates as $cert): ?>
        ...
        <article class="cert-card">
          ...
          <div class="cert-actions">
            <?php $accessActive = !$growerCertificatePaidRequired || $growerCertificatePaid; ?>
            <?php if ((string) $cert['status'] === 'issued' && !$expired): ?>
              <?php if ($accessActive): ?>
                ...
                <?php if ($htmlPath !== ''): ?><a class="button secondary" href="view-certificate.php?ref=<?= urlencode($displayRef) ?>" target="_blank" rel="noopener">View Online</a><?php endif; ?>
              <?php else: ?>
                <form method="post">
                  <input type="hidden" name="_csrf" value="<?= e(csrf_token()) ?>">
                  <input type="hidden" name="action" value="pay_grower_certificate">
                  <button type="submit" <?= $walletBalance >= $growerCertificateAmount ? '' : 'disabled' ?>>Pay Access / Renewal Fee</button>
                </form>
                <a class="button secondary" href="wallet.php?amount=<?= e((string) ceil($growerCertificateAmount)) ?>">Fund Wallet</a>
              <?php endif; ?>
            <?php endif; ?>
      ...
      <?php if (!$growerCertificates): ?>
        <div class="cert-empty">No grower certificate has been issued yet. When eligible, the issue and access payment action will appear above.</div>
      <?php endif; ?>
    </div>
  </section>
