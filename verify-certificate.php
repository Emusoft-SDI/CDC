...
function vc_lookup_certificate(PDO $pdo, string $ref): ?array
{
  ...
  if (app_table_exists($pdo, 'provider_accreditation_certificates')) {
    $certificate = provider_accreditation_certificate_by_ref($pdo, $ref);
    if ($certificate) {
      $certificate['certificate_program'] = 'NATCODEV Provider Accreditation';
      $certificate['certificate_type'] = 'Provider Accreditation Certificate';
      $certificate['subject_label'] = 'Provider';
      $certificate['app_ref'] = trim((string) ($certificate['company_name'] ?: $certificate['contact_person'] ?: 'Provider'));
      $certificate['name'] = trim((string) ($certificate['company_name'] ?: $certificate['contact_person'] ?: 'Provider'));
...
if ($canDownload && $certificate) {
  if (str_contains((string) ($certificate['certificate_type'] ?? ''), 'Grower')) {
    $downloadUrl = 'dashboard/download-certificate.php?ref=' . urlencode((string) $certificate['certificate_ref']);
  } elseif (str_contains((string) ($certificate['certificate_type'] ?? ''), 'Academy')) {
    $downloadUrl = 'dashboard/download-academy-certificate.php?ref=' . urlencode((string) $certificate['certificate_ref']);
  } elseif (str_contains((string) ($certificate['certificate_type'] ?? ''), 'Provider Accreditation')) {
    $downloadUrl = 'provider/download-accreditation-certificate.php';
  }
}
if ($canDownload && $certificate && str_contains((string) ($certificate['certificate_type'] ?? ''), 'Grower')) {
    $viewUrl = 'dashboard/view-certificate.php?ref=' . urlencode((string) $certificate['certificate_ref']);
}
...
$statusTitle = !$lookupAttempted ? 'Verify Any NATCODEV Certificate' : ($valid ? 'Certificate is Valid' : ($certificate ? ($revoked ? 'Certificate Has Been Revoked' : ($expired ? 'Certificate Has Expired' : 'Certificate is Not Valid')) : 'Certificate Not Found'));
...
<header class="top">
  ...
  <nav class="nav"><a href="index.php">Home</a><a href="market/index.php">Marketplace</a><a href="academy/index.php?screen=catalog">Academy</a><a class="active" href="verify-certificate.php">Certificates</a><a href="support/index.php?category=verification">Support</a></nav>
  <div class="top-actions"><a class="btn light" href="registry/index.php"><i class="fas fa-id-card"></i> Get Verified</a></div>
</header>
<section class="hero">
  <div>
...
<main class="wrap">
  <section class="result">
    <div class="status-head">
      <div class="status-left"><span class="status-icon <?= e($statusClass) ?>"><i class="fas <?= e($statusIcon) ?>"></i></span><div><h2><?= e($statusTitle) ?></h2><p class="muted"><?= $lookupAttempted ? ($valid ? 'This certificate is authentic and currently valid.' : ($revoked ? 'This certificate exists in NATCODEV records, but it has been revoked and is not valid.' : 'The reference was checked against NATCODEV records.')) : 'Enter a certificate reference or scan a QR code to confirm authenticity.' ?></p></div></div>
    ...
    <?php if ($certificate): ?>
      <div class="grid">
        <section class="panel details">
          ...
          <div class="detail"><strong><i class="fas fa-calendar-check"></i> Expiry Date</strong><span><?= e($expiryDate) ?><?= $expired ? ' <span class="badge">Expired</span>' : '' ?><?= $revoked ? ' <span class="badge">Revoked</span>' : '' ?></span></div>
          <div class="detail"><strong><i class="fas fa-shield-halved"></i> Registry Status</strong><span><?= e(ucwords(str_replace('_', ' ', (string) ($certificate['status'] ?? 'unknown')))) ?></span></div>
          <?php if ($revoked): ?><div class="detail"><strong><i class="fas fa-ban"></i> Revocation</strong><span><?= !empty($certificate['revoked_at']) ? e((string) $certificate['revoked_at']) . '<br>' : '' ?><?= e((string) ($certificate['revoked_reason'] ?? 'Revoked by registry authority.')) ?></span></div><?php endif; ?>
  ...
  <section class="guide">
    <div class="panel"><h3>Certificate Status Guide</h3><div class="guide-grid"><div class="mini"><i class="fas fa-circle-check"></i><h4>Valid</h4><p>This certificate is genuine and currently valid.</p></div><div class="mini expired"><i class="far fa-clock"></i><h4>Expired</h4><p>This certificate has expired and is no longer valid.</p></div><div class="mini revoked"><i class="fas fa-circle-xmark"></i><h4>Revoked</h4><p>This certificate has been revoked and is invalid.</p></div><div class="mini notfound"><i class="fas fa-circle-question"></i><h4>Not Found</h4><p>No certificate found with the reference provided.</p></div></div><p class="muted"><i class="far fa-clock"></i> Academy completion certificates do not expire. Operational credentials may have validity periods.</p></div>
