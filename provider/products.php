...
provider_simple_page('products', 'Products & Services', 'Add and manage approved products, services, and provider training programs.', function(PDO $pdo, array $user, array $provider): void {
    ...
    $sellerDashboardLink = null;
    foreach (market_user_dashboard_links($pdo, $user, 'provider') as $link) {
        if (($link['key'] ?? '') === 'seller') { $sellerDashboardLink = $link; break; }
    }
    if ($msg) {
    ...
    <div class="grid">
      ...
      <section class="card span-7"><div class="card-head"><h2>Current Listings</h2><?php if ($sellerDashboardLink): ?><a class="view" href="<?= e((string) $sellerDashboardLink['href']) ?>">Seller Central</a><?php else: ?><a class="view" href="support.php">Request Seller Store</a><?php endif; ?></div><div class="list">
