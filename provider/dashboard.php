...
<div class="grid" style="margin-top:16px">
  <section class="card span-4"><div class="card-head"><h2>Accreditation Checklist</h2><a class="view" href="accreditation.php">Details</a></div><div class="list">
    <?php $documents = provider_accreditation_documents($pdo, (int) $provider['id']); ?>
    <?php foreach (provider_accreditation_types() as $key => $item): $document = $documents[$key] ?? null; $status = strtolower(trim((string) ($document['status'] ?? 'not_uploaded'))); $badgeClass = $status === 'approved' ? '' : ($status === 'rejected' ? 'red' : 'warn'); ?>
      <div class="row"><span><i class="fas fa-lock" style="color:#08753a"></i> <?= e($item) ?><?php if ($document): ?> <small>(<?= e(provider_status_label($status)) ?>)</small><?php endif; ?></span><span class="badge <?= e($badgeClass) ?>"><?= e(provider_status_label($status)) ?></span></div>
    <?php endforeach; ?>
  </div></section>
