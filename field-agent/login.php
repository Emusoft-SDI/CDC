...
$error = '';
$notice = isset($_GET['message']) ? 'You have logged out of the field and farm workforce channel. Login again or return to NATCODEV.' : '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!app_check_rate_limit('field_agent_login', 8, 900)) {
        $error = 'Too many field operations login attempts. Please try again in 15 minutes.';
    } elseif (!verify_csrf($_POST['_csrf'] ?? null)) {
        $error = 'Please refresh and try again.';
    } else {
        $email = strtolower(trim((string) ($_POST['email'] ?? '')));
        $password = (string) ($_POST['password'] ?? '');
        $fields = ['id', 'name', 'email', 'password', 'role', 'account_status'];
        if (app_column_exists($pdo, 'users', 'platform_role')) {
            $fields[] = 'platform_role';
        }
        if (app_column_exists($pdo, 'users', 'email_verified_at')) {
        ...
        $isAllowedRole = $user ? app_user_has_any_role($pdo, $user, $fieldRoles) : false;
        if ($user && password_verify($password, (string) $user['password']) && $isAllowedRole) {
            if (app_user_needs_email_verification($user)) {
                $error = 'Confirm this staff email before entering Field Operations.';
            } elseif (!in_array((string) ($user['account_status'] ?? 'active'), ['active', 'verified'], true)) {
                $error = 'This field operations account is not active.';
            } else {
                $otpStart = otp_begin_email_login_challenge($pdo, $user, 'field-agent/' . field_login_home($user));
                if ($otpStart['ok']) {
            ...
            $error = 'Invalid field-operations email, password, or access role.';
        }
    }
}?>
<!DOCTYPE html>
...
<main class="login-shell">
  <section class="visual">
    <div>
      <h1>Field & Farm Workforce Login</h1>
      <p>Field agents, agronomists, extensionists, and farm hands enter here to complete practical field work and leave safely.</p>
  ...
  <section class="panel">
    ...
    <div class="card">
      ...
      <?php if ($notice): ?><div class="alert" style="background:#ecfdf3;color:#067647;border-color:#abefc6"><?= e($notice) ?></div><?php endif; ?><?php if ($error): ?><div class="alert"><?= e($error) ?></div><?php endif; ?>
      <form method="post">
        ...
        <div class="field">
          ...
          <button class="toggle" type="button" data-toggle-password="password" aria-pressed="false">Show</button>
        </div>
      ...
      <div class="links"><a href="../recruitment.php">Apply as Field Agent</a><a href="../dashboard/forgot-password.php">Forgot password?</a></div><div class="links"><a href="../index.php">NATCODEV Home</a><a href="../support/index.php">Support</a></div>
    </div>
  </section>
</main>
<script>document.querySelectorAll('[data-toggle-password]').forEach(function(button){button.addEventListener('click',function(){var input=document.getElementById(button.getAttribute('data-toggle-password'));if(!input)return;var show=input.type==='password';input.type=show?'text':'password';button.textContent=show?'Hide':'Show';button.setAttribute('aria-pressed',show?'true':'false');});});</script>
