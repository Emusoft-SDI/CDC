...
if (isset($_GET['oauth_callback'])) {
    try {
        ...
        error_log('Buyer registration OAuth error: ' . $e->getMessage());
        $error = $e->getMessage();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!app_check_rate_limit('buyer_register', 5, 900)) {
        $error = 'Too many buyer registration attempts. Please try again in 15 minutes.';
        ...
        if ($name === '' || !$email || strlen($password) < 6) {
            ...
            try {
                ...
                $message = 'Buyer account created. Check your email and verify before logging in.';
            } catch (Throwable $e) {
...
<main class="auth">
  ...
  <section class="panel">
    ...
    <?php if ($message): ?><div class="alert ok"><?= e($message) ?></div><?php endif; ?>
      ...
      <div class="links"><a href="../academy/dashboard.php">Back to Academy</a><a href="logout.php">Logout</a></div>
