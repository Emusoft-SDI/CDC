...
$galleryImages = market_listing_gallery_urls($item);
...
<section class="pd-wrap">
  <article class="pd-gallery">
    <div class="pd-main-img"><img id="productMainImage" src="<?= e($galleryImages[0] ?? market_listing_image_url($item)) ?>" alt="<?= e((string) $item['title']) ?>"></div>
    ...
    <div class="pd-thumbs" id="productThumbs">
      <?php foreach ($galleryImages as $index => $imageUrl): ?><button type="button" class="<?= $index === 0 ? 'active' : '' ?>" data-image="<?= e($imageUrl) ?>"><img src="<?= e($imageUrl) ?>" alt="<?= e((string) $item['title']) ?> image <?= $index + 1 ?>"></button><?php endforeach; ?>
    </div>
  </article>  <aside class="pd-info">
...
<section class="pd-tabs">
  ...
  <article class="pd-tab"><h3>Delivery & Coverage</h3><ul><li><?= e((string) ($item['location_label'] ?: $item['seller_location'] ?: 'Nigeria coverage')) ?></li><li><?= e(marketplace_status_label((string) ($item['fulfillment_method'] ?: 'seller_delivery'))) ?></li><?php if ((int)($item['pickup_available'] ?? 0) === 1): ?><li>Buyer pickup available</li><?php endif; ?><?php if ((int)($item['local_delivery_available'] ?? 0) === 1): ?><li>Local delivery available</li><?php endif; ?><?php if ((int)($item['nationwide_shipping_available'] ?? 0) === 1): ?><li>Nationwide shipping available</li><?php endif; ?><?php if ((string)($item['shipping_options'] ?? '') !== ''): ?><li><?= e((string)$item['shipping_options']) ?></li><?php endif; ?></ul><a class="mk-btn secondary" href="orders.php">Track Orders</a></article>
...
<script>
document.querySelectorAll('#productThumbs button[data-image]').forEach((button) => {
  button.addEventListener('click', () => {
    const image = document.getElementById('productMainImage');
    if (image) image.src = button.dataset.image || image.src;
    document.querySelectorAll('#productThumbs button').forEach((item) => item.classList.remove('active'));
