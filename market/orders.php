...
$canOpenBuyerDashboard = $user ? (bool) array_intersect(['buyer', 'consumer', 'user'], $userRoles) : false;
$accountFlash = $_SESSION['marketplace_checkout_account_flash'] ?? null;
if (is_array($accountFlash)) {
    unset($_SESSION['marketplace_checkout_account_flash']);
    $flashName = trim((string) ($accountFlash['name'] ?? ''));
    $flashEmail = trim((string) ($accountFlash['email'] ?? ''));
    if ($flashName !== '' || $flashEmail !== '') {
        $accountNotice = 'Your buyer account has been created. Check your email to verify it and then log in to track future orders faster.';
    }
}
    ...
    if (!$user) {
        ...
        try {
            ...
            $pdo->prepare("
                ...
                SET payment_status = 'paid', payment_method = 'wallet', status = 'paid', paid_at = COALESCE(paid_at, NOW())
...
if (isset($_GET['order_created'])) {
    $notice = 'Order placed successfully for bank transfer. Use your checkout reference (' . $displayCheckoutRef . ') and phone number to track payment and delivery status.';
}
...
market_header('Track Marketplace Order', 'orders', $pdo);
?>
  <style>
  ...
  <section class="wrap">
    ...
    <div class="grid">
      ...
      <aside class="side">
        ...
        <section class="panel paybox">
          ...
          <?php if ((string) ($primary['payment_method'] ?? '') === 'bank_transfer'): ?>
          <div style="margin:12px 0;padding:12px;background:#fff;border-radius:8px;border:1px dashed #bde4c5;">
            <strong style="color:var(--green)"><i class="fas fa-university"></i> Bank Transfer Details:</strong>
            <div style="margin-top:6px;font-size:.9rem;line-height:1.5;">
              <div>Bank: <strong><?= e(app_setting('bank_name', 'First Bank of Nigeria')) ?></strong></div>
              <div>Account Name: <strong><?= e(app_setting('bank_account_name', 'NATCODEV MARKETPLACE')) ?></strong></div>
              <div>Account Number: <strong><?= e(app_setting('bank_account_number', '3124567890')) ?></strong></div>
              <div>Payment Reference: <strong><?= e($displayCheckoutRef) ?></strong></div>
            </div>
          </div>
          <?php endif; ?>
          <div class="pay-actions">
            <?php if ($user): ?>
              ...
              <a class="btn" style="background:#fff;color:var(--green);border:1px solid var(--green)" href="../buyer/wallet.php"><i class="fas fa-plus-circle"></i> Fund Wallet</a>
            <?php else: ?>
        ...
        <?php endif; ?>
        <?php if ($accountNotice && (isset($_GET['paid']) || isset($_GET['order_created']))): ?>
        <section class="panel account-panel">
          <div class="summary-head" style="margin-bottom:8px">
            <div><h3><i class="fas fa-user-check" style="color:var(--green)"></i> Buyer Account Ready</h3></div>
            <span class="badge"><i class="fas fa-envelope"></i> Verify Email</span>
          </div>
          <p><?= e($accountNotice) ?></p>
          <div class="acct-meta">
            <span class="acct-pill"><i class="fas fa-receipt"></i> Order tracked by reference</span>
            <span class="acct-pill"><i class="fas fa-truck"></i> Faster future tracking</span>
          </div>
          <div class="acct-actions">
            <a class="btn" href="../buyer/login.php"><i class="fas fa-right-to-bracket"></i> Login now</a>
            <a class="btn" style="background:#fff;color:var(--green);border:1px solid var(--green)" href="index.php"><i class="fas fa-store"></i> Continue shopping</a>
          </div>
        </section>
        <?php endif; ?>
        <?php if ((isset($_GET['paid']) || isset($_GET['order_created'])) && $canOpenBuyerDashboard): ?>
        <section class="panel account-panel">
          <div class="summary-head" style="margin-bottom:8px">
            <div><h3><i class="fas fa-user" style="color:var(--green)"></i> Buyer Dashboard Ready</h3></div>
            <span class="badge"><i class="fas fa-check-circle"></i> Signed In</span>
          </div>
          <p>Your buyer workspace is ready. Open it now to view your orders, wallet, refunds, support, and profile details.</p>
          <div class="acct-meta">
            <span class="acct-pill"><i class="fas fa-box"></i> Track orders</span>
            <span class="acct-pill"><i class="fas fa-wallet"></i> Manage wallet</span>
          </div>
          <div class="acct-actions">
            <a class="btn" href="../buyer/index.php"><i class="fas fa-right-to-bracket"></i> Open Buyer Dashboard</a>
            <a class="btn" style="background:#fff;color:var(--green);border:1px solid var(--green)" href="index.php"><i class="fas fa-store"></i> Continue shopping</a>
          </div>
        </section>
        <?php endif; ?>
    ...
    <div class="grid" style="margin-top:16px">
      ...
      <aside class="side">
        <section class="panel"><h3><i class="fas fa-headset" style="color:var(--green)"></i> Need Help?</h3><p>Our support team is here to help you.</p><div class="contact-row"><i class="fas fa-phone"></i> <?= e($office['phone_display']) ?></div><div class="contact-row"><i class="fas fa-location-dot"></i> <?= e(implode(', ', $office['address_lines'])) ?></div><div class="contact-row"><i class="fas fa-envelope"></i> support@natcodev.com.ng</div><a class="btn" style="width:100%;background:#fff;color:var(--green);border:1px solid var(--green)" href="../support/index.php?category=marketplace"><i class="fas fa-comments"></i> Open Support Ticket</a></section>
      </aside>
    </div>
    <?php else: ?>
      <section class="panel">
