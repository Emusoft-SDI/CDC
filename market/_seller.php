        ...
        if (!verify_csrf($_POST['_csrf'] ?? null)) {
            ...
            try {
                ...
                if ($action === 'save_seller') {
                    ...
                    $uploadedLogoPath = market_upload_seller_logo('store_logo');
                    if ($seller) {
                        ...
                        $stmt = $pdo->prepare("
                            ...
                            SET seller_type=?, store_name=?, slug=?, description=?, contact_person=?, email=?, phone=?, whatsapp=?,
                                location_label=?, coverage_area=?, fulfillment_options=?, logo_path=COALESCE(?, logo_path), approval_status=IF(approval_status='approved','approved','pending')
                            WHERE id=? AND user_id=?
                        ");
                        $stmt->execute([$sellerType, $storeName, $slug, trim((string) ($_POST['description'] ?? '')), trim((string) ($_POST['contact_person'] ?? '')), trim((string) ($_POST['email'] ?? '')), trim((string) ($_POST['phone'] ?? '')), trim((string) ($_POST['whatsapp'] ?? '')), trim((string) ($_POST['location_label'] ?? '')), trim((string) ($_POST['coverage_area'] ?? '')), trim((string) ($_POST['fulfillment_options'] ?? '')), $uploadedLogoPath, (int) $seller['id'], $userId]);
                        $message = 'Seller profile updated.';
                    } else {
                        $slug = marketplace_unique_slug($pdo, 'marketplace_sellers', $storeName);
                        $stmt = $pdo->prepare("
                            INSERT INTO marketplace_sellers
                                (user_id, seller_type, store_name, slug, description, contact_person, email, phone, whatsapp, location_label, coverage_area, fulfillment_options, logo_path)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ");
                        $stmt->execute([$userId, $sellerType, $storeName, $slug, trim((string) ($_POST['description'] ?? '')), trim((string) ($_POST['contact_person'] ?? '')), trim((string) ($_POST['email'] ?? '')), trim((string) ($_POST['phone'] ?? '')), trim((string) ($_POST['whatsapp'] ?? '')), trim((string) ($_POST['location_label'] ?? '')), trim((string) ($_POST['coverage_area'] ?? '')), trim((string) ($_POST['fulfillment_options'] ?? '')), $uploadedLogoPath]);
                ...
                if ($action === 'save_listing' && $seller) {
                    ...
                    $newImagePaths = array_values(array_filter(array_merge($uploadedImagePath ? [$uploadedImagePath] : [], $uploadedGalleryPaths)));
                    $existingImagePaths = $existingListing ? market_listing_gallery_paths($existingListing) : [];
                    $listingImagePaths = array_values(array_unique(array_slice(array_merge($newImagePaths, $existingImagePaths), 0, 4)));
                    $mainImagePath = $listingImagePaths[0] ?? null;
                    $galleryImagePaths = $mainImagePath ? array_values(array_filter($listingImagePaths, static fn(string $path): bool => $path !== $mainImagePath)) : [];
                    $shippingOptions = trim((string) ($_POST['shipping_options'] ?? ''));
                    $pickupAvailable = isset($_POST['pickup_available']) ? 1 : 0;
                    $localDeliveryAvailable = isset($_POST['local_delivery_available']) ? 1 : 0;
                    $nationwideShippingAvailable = isset($_POST['nationwide_shipping_available']) ? 1 : 0;
                    ...
                    $payload = [
                        ...
                        trim((string) ($_POST['mpn'] ?? '')), trim((string) ($_POST['origin_country'] ?? '')), trim((string) ($_POST['manufacturer'] ?? '')), trim((string) ($_POST['brand'] ?? '')),
                    ...
                    if ($listingId > 0) {
                        ...
                        $stmt = $pdo->prepare("
                            ...
                            SET category_id=?, listing_type=?, title=?, summary=?, description=?, price=?, price_unit=?, quantity_available=?,
                                ...
                                mpn=?, origin_country=?, manufacturer=?, brand=?, model_number=?, tags=?, requires_shipping=?, downloadable=?, slug=?
                            WHERE id=? AND seller_id=?
                        ");
                        $stmt->execute([...$payload, $slug, $listingId, (int) $seller['id']]);
                        $pdo->prepare("UPDATE marketplace_listings SET image_path = COALESCE(?, image_path), image_gallery = ?, shipping_options = ?, pickup_available = ?, local_delivery_available = ?, nationwide_shipping_available = ? WHERE id = ? AND seller_id = ?")->execute([$mainImagePath, json_encode($galleryImagePaths, JSON_UNESCAPED_SLASHES), $shippingOptions, $pickupAvailable, $localDeliveryAvailable, $nationwideShippingAvailable, $listingId, (int) $seller['id']]);
                        $message = 'Listing updated and sent for approval.';
                    } else {
                        $slug = marketplace_unique_slug($pdo, 'marketplace_listings', $title);
                        $stmt = $pdo->prepare("
                            INSERT INTO marketplace_listings
                                (seller_id, category_id, listing_type, title, slug, summary, description, price, price_unit, quantity_available, unit, min_order_quantity, location_label, fulfillment_method, availability_status, approval_status, mpn, origin_country, manufacturer, brand, model_number, tags, requires_shipping, downloadable)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?, ?, ?)
                        ");
                        $stmt->execute([(int) $seller['id'], $categoryId, $listingType, $title, $slug, ...array_slice($payload, 3)]);
                        $newListingId = (int) $pdo->lastInsertId();
                        $pdo->prepare("UPDATE marketplace_listings SET image_path = ?, image_gallery = ?, shipping_options = ?, pickup_available = ?, local_delivery_available = ?, nationwide_shipping_available = ? WHERE id = ? AND seller_id = ?")->execute([$mainImagePath, json_encode($galleryImagePaths, JSON_UNESCAPED_SLASHES), $shippingOptions, $pickupAvailable, $localDeliveryAvailable, $nationwideShippingAvailable, $newListingId, (int) $seller['id']]);
...
function seller_access_or_message(PDO $pdo, array $user): void
{
    ...
    <section class="sc-card sc-panel">
      ...
      <p>Your current profile can browse the marketplace, but seller access has not been activated for this account yet.</p>
      <div class="sc-actions"><a class="sc-btn" href="seller-register.php">Activate Seller Access</a><a class="sc-btn secondary" href="index.php">Open Marketplace</a><a class="sc-btn secondary" href="<?= e(market_dashboard_url_for_user($pdo, $user, 'index.php')) ?>">Back to My Workspace</a></div>
    </section>
    <?php
    seller_footer();
    exit;
...
function seller_header(string $title, string $active, array $user, ?array $seller): void
{
    $menu = [
        ...
        'support' => ['Support', 'seller-support.php', 'headphones'],
    ...
    $dashboardLinks = market_user_dashboard_links(db(), $user, 'seller');
    ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="sc-shell">
  <aside class="sc-side">
    <a class="sc-brand" href="index.php"><img src="<?= e(app_primary_logo_url()) ?>" alt="NATCODEV"><span><strong>NATCODEV</strong><span>NATCODEV Marketplace<br>Seller Central</span></span></a>
    ...
    <div class="sc-upgrade"><h3>Role Access</h3><p>Move only between dashboards enabled for this account.</p><?php foreach ($dashboardLinks as $dash): ?><a class="sc-btn secondary" style="margin:6px 6px 0 0" href="<?= e((string) $dash['href']) ?>"><?= e((string) $dash['label']) ?></a><?php endforeach; ?></div>
  </aside>
  <section class="sc-main">
    <header class="sc-top">
      ...
      <?php foreach ($dashboardLinks as $dash): ?><a class="sc-chip <?= !empty($dash['active']) ? 'active' : '' ?>" href="<?= e((string) $dash['href']) ?>"><i data-lucide="<?= e((string) $dash['icon']) ?>"></i><?= e((string) $dash['label']) ?></a><?php endforeach; ?>
      ...
      <a class="sc-chip" href="seller-payouts.php"><i data-lucide="wallet"></i> Wallet</a>
      <a class="sc-chip" href="seller-settings.php"><i data-lucide="user"></i> Profile</a>
      <a class="sc-chip" href="seller-settings.php#account"><i data-lucide="settings"></i> Account</a>
      <a class="sc-chip" href="logout.php"><i data-lucide="log-out"></i> Logout</a>
    </header>
...
function seller_kpis(array $ctx): void
{
    ...
    <section class="sc-kpis">
      <div class="sc-card sc-kpi"><span class="sc-icon"><i data-lucide="wallet"></i></span><div><small>Todayâ€™s Sales</small><b><?= e(marketplace_money($sales)) ?></b><span>+ active revenue</span></div></div>
