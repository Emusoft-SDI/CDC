...
<section class="sc-card sc-panel">
  ...
  <form method="post" enctype="multipart/form-data" class="sc-form sc-form-grid">
    ...
    <div class="wide"><label>Main Product Image</label><?php if ($edit && market_listing_gallery_urls($edit)): ?><div style="display:flex;gap:10px;flex-wrap:wrap;margin:8px 0 12px"><?php foreach (market_listing_gallery_urls($edit) as $imageUrl): ?><img src="<?= e($imageUrl) ?>" style="width:120px;height:92px;object-fit:cover;border-radius:10px;border:1px solid var(--line)" alt=""><?php endforeach; ?></div><?php endif; ?><input type="file" name="listing_image" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp"><small class="muted">This becomes the first buyer-facing image.</small></div>
    <div class="wide"><label>Additional Product Images</label><input type="file" name="listing_images[]" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" multiple><small class="muted">Upload up to 3 more images for the buyer gallery.</small></div>
    <div><label>MPN / SKU</label><input name="mpn" value="<?= e((string) ($edit['mpn'] ?? '')) ?>"></div><div><label>Origin Country</label><input name="origin_country" value="<?= e((string) ($edit['origin_country'] ?? 'Nigeria')) ?>"></div>
    ...
    <div class="wide"><label>Fulfillment Method</label><select name="fulfillment_method"><?php foreach (['seller_delivery'=>'Seller delivery','buyer_pickup'=>'Buyer pickup','platform_delivery'=>'NATCODEV/platform delivery','scheduled_service'=>'Scheduled service/installation','digital_delivery'=>'Digital/download delivery'] as $value=>$label): ?><option value="<?= e($value) ?>" <?= (string) ($edit['fulfillment_method'] ?? 'seller_delivery') === $value ? 'selected' : '' ?>><?= e($label) ?></option><?php endforeach; ?></select><small class="muted">How this product or service reaches the buyer after checkout.</small></div>
    <div class="wide"><label>Shipping Notes / Coverage</label><textarea name="shipping_options" placeholder="Delivery cities, pickup address, dispatch timelines, shipping fees, cold-chain needs, installation schedule..."><?= e((string) ($edit['shipping_options'] ?? '')) ?></textarea></div>
    <label><input type="checkbox" name="requires_shipping" <?= (int) ($edit['requires_shipping'] ?? 1) === 1 ? 'checked' : '' ?>> Physical item requires delivery handling</label><label><input type="checkbox" name="pickup_available" <?= (int) ($edit['pickup_available'] ?? 0) === 1 ? 'checked' : '' ?>> Buyer pickup available</label><label><input type="checkbox" name="local_delivery_available" <?= (int) ($edit['local_delivery_available'] ?? 0) === 1 ? 'checked' : '' ?>> Local delivery available</label><label><input type="checkbox" name="nationwide_shipping_available" <?= (int) ($edit['nationwide_shipping_available'] ?? 0) === 1 ? 'checked' : '' ?>> Nationwide shipping available</label><label><input type="checkbox" name="downloadable" <?= (int) ($edit['downloadable'] ?? 0) === 1 ? 'checked' : '' ?>> Downloadable / digital item</label>
