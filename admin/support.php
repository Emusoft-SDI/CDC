...
if (!function_exists('sd_minutes_label')) {
...
if (!function_exists('sd_short_date')) {
...
if (!function_exists('sd_agent_label')) {
...
if (!function_exists('sd_ticket_url')) {
...
if (!function_exists('sd_pagination_controls')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['_csrf'] ?? null)) {
        ...
        try {
            ...
            // Strict platform governance: tickets are immutable dispute and support audit records
            if (in_array(strtolower($action), ['delete', 'delete_ticket', 'destroy', 'remove', 'purge', 'drop', 'edit_requester', 'edit_inquiry'], true)) {
                throw new RuntimeException('Arbitrary CRUD operations and ticket deletion are prohibited by platform governance. Support tickets are immutable legal audit records. Use lifecycle status transitions (e.g. Resolved, Closed, or Rejected) to conclude tickets.');
            }
            if (!in_array($action, ['create_team', 'manage_lifecycle', 'update_ticket'], true)) {
                throw new RuntimeException('Unsupported action: ' . htmlspecialchars($action));
            }
            ...
            if ($action === 'create_team') {
                ...
                $message = 'Ticket lifecycle updated and audit trail recorded.';
            }
        } catch (Throwable $e) {
            $error = $e->getMessage();
        }
...
if ($workspaceView === 'complaints' && $filterCategory === '') {
    $where[] = "(category = 'general' OR subject LIKE '%complaint%' OR description LIKE '%complaint%')";
} elseif ($filterCategory !== '' && isset($categories[$filterCategory])) {
    $where[] = 'category = ?';
...
if (!defined('NATCODEV_SUPPORT_WORKSPACE_VIEW')) {
