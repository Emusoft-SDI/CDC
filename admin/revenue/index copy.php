...
$allowed = ['overview','rules','ledger','marketplace','plans','promotions','registry','provider-fees','integrity'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'save_rule') {
            ...
            $message = 'Certificate access and renewal fees saved.'; $page = 'registry';
...
// Wallet integrity data
$walletTotal = (int) rev_scalar($pdo, 'SELECT COUNT(*) FROM wallets');
$walletOwnerTotal = (int) rev_scalar($pdo, 'SELECT COUNT(DISTINCT user_id) FROM wallets');
$duplicateWalletRows = max(0, $walletTotal - $walletOwnerTotal);
$balance = rev_scalar($pdo, 'SELECT COALESCE(SUM(balance),0) FROM wallets');
$ledgerNet = rev_scalar($pdo, "SELECT COALESCE(SUM(CASE WHEN type='credit' AND status IN ('completed','successful','success','paid') THEN amount WHEN type IN ('debit','withdrawal') AND status IN ('completed','successful','success','paid') THEN -ABS(amount) ELSE 0 END),0) FROM wallet_transactions");
$walletVariance = $balance - $ledgerNet;
$walletIntegrityRows = rev_rows($pdo, "SELECT u.id,u.name,u.email,COUNT(w.id) wallet_rows,GROUP_CONCAT(w.id ORDER BY w.id) wallet_ids,COALESCE(SUM(w.balance),0) wallet_balance,COALESCE(SUM(w.hold_balance),0) hold_balance,(SELECT COUNT(*) FROM wallet_transactions wt WHERE wt.user_id=u.id) tx_count,(SELECT COALESCE(SUM(CASE WHEN wt.type='credit' AND wt.status IN ('completed','successful','success','paid') THEN wt.amount WHEN wt.type IN ('debit','withdrawal') AND wt.status IN ('completed','successful','success','paid') THEN -ABS(wt.amount) ELSE 0 END),0) FROM wallet_transactions wt WHERE wt.user_id=u.id) ledger_net FROM wallets w JOIN users u ON u.id=w.user_id GROUP BY u.id,u.name,u.email HAVING COUNT(w.id)>1 ORDER BY wallet_rows DESC,u.name ASC LIMIT 100");
...
$rules = rev_rows($pdo, "SELECT * FROM platform_fee_rules ORDER BY source_module, sort_order, id");
$plans = rev_rows($pdo, "SELECT * FROM seller_subscription_plans ORDER BY sort_order, monthly_fee");
      ...
      <div class="card-body">
        ...
        <form method="post">
          ...
          <div class="row g-3">
            ...
            <?php foreach (grower_member_types() as $type => $label): $fee = (float) ($certificateFeeConfig['fees'][$type] ?? 0); $validity = (int) ($certificateFeeConfig['validity_months'][$type] ?? 12); ?>
            <div class="col-md-6">
...
<?php endif; ?>
<?php if ($page === 'provider-fees'): ?><div class="card mb-3"><div class="card-body"><h2 class="h5">Provider Accreditation Fee Report Card</h2><p class="text-secondary">Current provider accreditation fee settings and recent paid access activity.</p><p class="text-secondary small mb-3">Platform earnings from provider accreditation and grower certificate access/renewal fees are retained in the NATCODEV platform revenue ledger and separate from user wallet balances.</p><div class="row g-3 mt-3"><div class="col-md-4"><div class="card h-100"><div class="card-body"><small class="text-secondary">Enabled</small><div class="h4"><?= $providerCertificateFeeConfig['enabled'] ? 'Yes' : 'No' ?></div></div></div></div><div class="col-md-4"><div class="card h-100"><div class="card-body"><small class="text-secondary">Access Fee</small><div class="h4 money"><?= rev_e(rev_money((float) $providerCertificateFeeConfig['amount'])) ?></div></div></div></div><div class="col-md-4"><div class="card h-100"><div class="card-body"><small class="text-secondary">Validity Months</small><div class="h4"><?= (int) $providerCertificateFeeConfig['validity_months'] ?></div></div></div></div></div><div class="row g-3 mt-3"><div class="col-md-4"><div class="card h-100"><div class="card-body"><small class="text-secondary">Total Revenue</small><div class="h4 money"><?= rev_e(rev_money((float) $providerCertificateAccessRevenue)) ?></div></div></div></div><div class="col-md-4"><div class="card h-100"><div class="card-body"><small class="text-secondary">Paid Access Count</small><div class="h4"><?= number_format($providerCertificateAccessCount) ?></div></div></div></div><div class="col-md-4"><div class="card h-100"><div class="card-body"><small class="text-secondary">Last Refresh</small><div class="h4"><?= rev_e(date('Y-m-d H:i:s')) ?></div></div></div></div></div></div></div><div class="card"><div class="card-body"><h3 class="h5">Recent Provider Accreditation Payments</h3><div class="table-responsive"><table class="table mb-0"><tr><th>Date</th><th>Reference</th><th>User ID</th><th>Provider ID</th><th>Amount</th><th>Valid Until</th></tr><?php foreach ($providerAccPayments as $payment): ?><tr><td><?= rev_e($payment['paid_at'] ?? '') ?></td><td><?= rev_e($payment['reference'] ?? '') ?></td><td><?= (int) ($payment['user_id'] ?? 0) ?></td><td><?= (int) ($payment['provider_id'] ?? 0) ?></td><td><?= rev_e(rev_money((float) ($payment['amount'] ?? 0))) ?></td><td><?= rev_e($payment['valid_until'] ?? '') ?></td></tr><?php endforeach; ?><?php if (!$providerAccPayments): ?><tr><td colspan="6" class="text-center text-secondary py-4">No provider accreditation payments found yet.</td></tr><?php endif; ?></table></div></div></div><?php endif; ?>
