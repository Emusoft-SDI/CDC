...
$params = [];
if ($search !== '') {
    $where .= ' AND (a.app_ref LIKE ? OR a.name LIKE ? OR a.email LIKE ? OR a.phone LIKE ? OR a.location LIKE ?)';
    $term = '%' . $search . '%';
    $params = [$term, $term, $term, $term, $term];
}

$total = rx_scalar($pdo, "SELECT COUNT(*) FROM applications a WHERE {$where}", $params);
$apps = rx_rows($pdo, "
...
<div class="card"><div class="card-header"><form method="get" style="display:flex;gap:10px;flex:1"><input class="form-input" name="search" value="<?= rx_e($search) ?>" placeholder="Search reference, applicant, email, phone, or location"><input type="hidden" name="per_page" value="<?= $limit ?>"><button class="btn btn-secondary">Search</button><?php if($search!==''): ?><a class="btn btn-secondary" href="applications.php">Clear</a><?php endif; ?></form></div></div>
      ...
      <tbody>
        <?php foreach ($apps as $row): ?>
          <tr>
            ...
            <td>
              <button type="button" class="btn btn-sm btn-primary" onclick='openReviewModal(<?= (int)$row['id'] ?>, <?= json_encode((string)$row['name'], JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'>Review</button>
            </td>
          </tr>
        <?php endforeach; ?>
