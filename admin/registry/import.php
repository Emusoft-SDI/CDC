...
$totalImports = rx_scalar($pdo, "SELECT COUNT(*) FROM user_import_records WHERE {$where}", $params);
...
<div class="card">
    <div class="card-header" style="gap:12px;flex-wrap:wrap"><h3 class="card-title">Audit Log: Import Rows</h3><form method="get" style="display:flex;gap:8px;flex:1;justify-content:flex-end;flex-wrap:wrap"><input class="form-input" name="search" value="<?= rx_e($search) ?>" placeholder="Search name, contact, batch, or note" style="max-width:290px"><select class="form-select" name="status" style="width:auto"><option value="">All statuses</option><?php foreach($statuses as $item): ?><option value="<?= rx_e($item['status']) ?>" <?= $status===$item['status']?'selected':'' ?>><?= rx_e(ucwords(str_replace('_',' ',$item['status']))) ?></option><?php endforeach; ?></select><select class="form-select" name="role" style="width:auto"><option value="">All roles</option><?php foreach($roles as $item): ?><option value="<?= rx_e($item['role']) ?>" <?= $role===$item['role']?'selected':'' ?>><?= rx_e(ucwords(str_replace('_',' ',$item['role']))) ?></option><?php endforeach; ?></select><input type="hidden" name="per_page" value="<?= $limit ?>"><button class="btn btn-secondary">Filter</button><a class="btn btn-secondary" href="import.php">Reset</a></form></div>
...
<?= rx_pagination_links($totalImports, $limit, $page, 'import.php') ?>
