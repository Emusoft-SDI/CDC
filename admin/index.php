...
admin_handle_logout_request('login.php');
if (!admin_session_is_authenticated($pdo)) {
    redirect_to('admin.php');
}
if (!empty($_GET['ticket'])) {
    $ticketRef = preg_replace('/[^A-Z0-9-]/i', '', (string) $_GET['ticket']);
    redirect_to('support/?' . http_build_query(['ticket' => $ticketRef]) . '#ticket-workbench');
}
...
$workspaceProfileKey = preg_replace('/[^a-z0-9_-]/i', '', $role) ?: 'admin';

function admin_hub_href_feature(string $href): ?string
{
    $href = trim(str_replace('\\', '/', $href));
    if ($href === '' || str_starts_with($href, '#')) {
        return null;
    }
    if (str_starts_with($href, '../super-admin/')) {
    ...
    if (isset($folderFeatures[$first])) {
        return $folderFeatures[$first];
    }
    return admin_feature_for_script(basename($path));
}

function admin_hub_href_allowed(PDO $pdo, string $href, bool $isSuperAdmin): bool
{
    $feature = admin_hub_href_feature($href);
    if ($feature === '__super_admin__') {
        return $isSuperAdmin;
    }
    if ($feature === null) {
        return true;
    }
    return admin_feature_is_allowed($pdo, $feature);
}

function admin_hub_filter_sections(PDO $pdo, array $sections, bool $isSuperAdmin): array
{
    return array_values(array_filter($sections, static function (array $section) use ($pdo, $isSuperAdmin): bool {
...
if (app_table_exists($pdo, 'dr_backups')) {
    try {
        $latestBackupStatus = (string) ($pdo->query("SELECT status FROM dr_backups ORDER BY created_at DESC, id DESC LIMIT 1")->fetchColumn() ?: 'No backups yet');
    } catch (Throwable $e) {
        $latestBackupStatus = 'Review needed';
    }
}
$siteZipReady = class_exists('ZipArchive') ? 'ZIP Ready' : 'ZIP Off';
...
$topNotifications = [
    ...
    ['label' => 'Delete approvals', 'value' => $pendingDeleteApprovals, 'href' => '../super-admin/index.php?view=approvals', 'super_only' => true],
];
$topMessages = [
    ...
    ['label' => 'Applications pending', 'value' => $pendingApplications, 'href' => 'registry/?page=applications'],
];
$quickCommands = [
    ['label' => 'Search Everything', 'href' => 'search.php', 'feature' => 'dashboard'],
    ['label' => 'Import Users', 'href' => 'import-users.php', 'feature' => 'imports', 'super_only' => true],
    ['label' => 'Export Registry', 'href' => 'admin.php?export=1', 'super_only' => true],
    ['label' => 'Generate Reports', 'href' => 'reports.php?page=exports', 'feature' => 'reports'],
    ['label' => 'Review Approvals', 'href' => '../super-admin/index.php?view=approvals', 'super_only' => true],
];
$profileLinks = [
    ['label' => 'My Profile', 'href' => 'profile.php?workspace=' . rawurlencode($workspaceProfileKey), 'icon' => 'fa-user', 'feature' => 'profile'],
    ['label' => 'Admin Settings', 'href' => 'settings.php', 'icon' => 'fa-gear', 'feature' => 'settings'],
    ['label' => 'Super Admin', 'href' => '../super-admin/index.php', 'icon' => 'fa-shield-halved', 'super_only' => true],
];
$profileLinks = array_values(array_filter($profileLinks, static fn (array $item): bool => (empty($item['super_only']) || $isSuperAdmin) && admin_hub_href_allowed($pdo, (string) ($item['href'] ?? ''), $isSuperAdmin)));
...
$workspaceCatalog = [
    ...
    [
        ...
        'sections' => [
            ...
            ...($isSuperAdmin ? [['Legacy Registry', 'admin.php']] : []),
        ],
    ],
    [
        'key' => 'marketplace',
    ...
    [
        ...
        'sections' => [
            ['Overview', 'academy/'],
            ['Programs', 'academy/?page=programs'],
            ['Courses', 'academy/?page=courses'],
            ['Lessons', 'academy/?page=lessons'],
            ['Materials', 'academy/?page=lessons'],
            ['Assessments', 'academy/?page=assessments'],
            ['Question Bank', 'academy/?page=assessments'],
            ['Cohorts', 'academy/?page=calendar'],
            ['Learners', 'academy/?page=enrollments'],
            ['Progress', 'academy/?page=reports'],
            ['Attempts', 'academy/?page=reports'],
            ['Certificates', 'academy/?page=certificates'],
            ['Reports', 'academy/?page=reports'],
        ],
    ],
    [
        'key' => 'wallet',
        ...
        'metrics' => [['Available Balance', hub_money($walletBalance)], ['Debit Exposure', hub_money($walletDebitExposure)], ['Settlements Pending', $settlementsPending]],
        'tone' => 'green',
        'status' => 'Operational',
        'sections' => [
    ...
    [
        ...
        'feature' => 'backups',
        'href' => 'backups.php',
        'text' => 'Create full-site ZIP archives, SQL dumps, offsite copies, and restore evidence.',
        'metrics' => [['Backup Bundles', $backupCount], ['Latest Status', status_label($latestBackupStatus)], ['Site Archive', $siteZipReady]],
        'tone' => $backupCount > 0 ? 'green' : 'orange',
        'status' => $backupCount > 0 ? 'Operational' : 'Needs Backup',
        'sections' => [
            ['Backup Control Center', 'backups.php'],
            ['Create Backup Bundle', 'backups.php#create'],
            ['Recent Evidence', 'backups.php#evidence'],
            ['Restore Runbook', 'backups.php#restore-runbook'],
...
$workspaceCatalog = array_map(static function (array $item) use ($pdo, $isSuperAdmin): array {
    $item['sections'] = admin_hub_filter_sections($pdo, (array) ($item['sections'] ?? []), $isSuperAdmin);
    return $item;
}, $workspaceCatalog);
$workspaces = array_values(array_filter($workspaceCatalog, static fn(array $item): bool => admin_feature_is_allowed($pdo, (string) $item['feature']) && admin_hub_href_allowed($pdo, (string) ($item['href'] ?? ''), $isSuperAdmin)));
$kpis = [
...
<div class="hub">
  <aside class="side">
    ...
    <div class="quick">
      ...
      <nav class="nav">
        <?php if (admin_feature_is_allowed($pdo, 'applications')): ?><a href="registry/?page=applications"><i class="fas fa-user-plus"></i> Add New Grower</a><?php endif; ?>
        ...
        <?php if (admin_feature_is_allowed($pdo, 'training')): ?><a href="academy/"><i class="fas fa-calendar-check"></i> Manage Courses</a><?php endif; ?>
      </nav>
    </div>
    ...
    <header class="top">
      ...
      <div class="top-actions">
        <div class="top-menu">
          ...
          <div class="dropdown">
            <h3>Notifications</h3>
            <?php foreach ($topNotifications as $item): ?><?php if ((empty($item['super_only']) || $isSuperAdmin) && admin_hub_href_allowed($pdo, (string) $item['href'], $isSuperAdmin)): ?><a href="<?= e($item['href']) ?>"><span><?= e($item['label']) ?><small>Needs attention</small></span><span class="count"><?= (int) $item['value'] ?></span></a><?php endif; ?><?php endforeach; ?>
            <?php if (admin_hub_href_allowed($pdo, 'notifications.php', $isSuperAdmin)): ?><a href="notifications.php"><span>Notification log<small>Delivery and template events</small></span><i class="fas fa-arrow-right"></i></a><?php endif; ?>
          </div>
        </div>
        <div class="top-menu">
          ...
          <div class="dropdown">
            ...
            <?php foreach ($topMessages as $item): ?><?php if (admin_hub_href_allowed($pdo, (string) $item['href'], $isSuperAdmin)): ?><a href="<?= e($item['href']) ?>"><span><?= e($item['label']) ?><small>Live workspace queue</small></span><span class="count"><?= (int) $item['value'] ?></span></a><?php endif; ?><?php endforeach; ?>
          </div>
        </div>
        <div class="top-menu command-menu">
          ...
          <div class="dropdown">
            <h3>Quick Command</h3>
            <?php foreach ($quickCommands as $item): ?><?php if ((empty($item['super_only']) || $isSuperAdmin) && admin_hub_href_allowed($pdo, (string) $item['href'], $isSuperAdmin)): ?><a href="<?= e($item['href']) ?>"><span><?= e($item['label']) ?></span><i class="fas fa-arrow-right"></i></a><?php endif; ?><?php endforeach; ?>
          </div>
        </div>
      ...
      <div class="layout">
        <section>
          ...
          <section class="health-panel">
            <div class="rail-head"><h2>System Health</h2><?php if (admin_hub_href_allowed($pdo, 'monitoring.php', $isSuperAdmin)): ?><a href="monitoring.php">View Details</a><?php endif; ?></div>
          ...
          <section class="rail-card activity-section">
            <div class="rail-head"><h2>Today's Activity</h2><button type="button" id="activityCollapse">Collapse</button><?php if (admin_hub_href_allowed($pdo, 'reports.php', $isSuperAdmin)): ?><a href="reports.php">View All</a><?php endif; ?></div>
