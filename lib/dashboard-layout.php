...
function dashboard_nav_items(): array
{
    return [
        ...
        ['href' => '../market/seller-central.php', 'label' => 'Seller Central', 'roles' => ['seller','marketplace_seller']],
...
function dashboard_allowed_nav_groups(PDO $pdo): array
{
    ...
    $roleKeys = $user ? market_user_role_keys($pdo, $user) : [];

    foreach (dashboard_nav_groups() as $group) {
        $items = array_values(array_filter($group['items'], static function (array $item) use ($pdo, $isAdminUser, $isSuperUser, $roleKeys): bool {
            ...
            if (!empty($item['roles']) && !array_intersect((array) $item['roles'], $roleKeys)) {
                return false;
            }
...
function dashboard_page_start(string $title, array $options = []): void
{
    ...
    if (session_status() === PHP_SESSION_ACTIVE && (!isset($options['user_name']) || !array_key_exists('unread', $options) || !array_key_exists('profile_picture', $options) || !array_key_exists('notices', $options) || !array_key_exists('location', $options))) {
        try {
            ...
            if ($current && !isset($options['dashboard_links'])) {
                $options['dashboard_links'] = market_user_dashboard_links($pdo, $current, 'grower');
            }
  ...
  <div class="tb-right">
    ...
    <div class="tb-user-menu" id="topUserMenu">
      ...
      <div class="tb-dropdown" role="menu" aria-label="Account menu">
        ...
        <a class="tb-drop-link" href="account-settings.php"><i class="fas fa-cog"></i> Account Settings</a><?php foreach ((array)($options['dashboard_links'] ?? []) as $dash): ?><a class="tb-drop-link" href="<?= e((string) $dash['href']) ?>"><i class="fas fa-random"></i> <?= e((string) $dash['label']) ?></a><?php endforeach; ?>
