...
function fm_ensure_schema(PDO $pdo): void
{
    ...
    app_add_column_if_missing($pdo, 'farm_visits', 'photos', 'TEXT NULL');
...
function fm_record_task_visit(PDO $pdo, array $user, array $payload): array
{
    ...
    $evidenceFiles = $payload['evidence_files'] ?? $payload['photos'] ?? null;
    $photosJson = null;
    if (is_array($evidenceFiles)) {
        $photosJson = json_encode(array_values($evidenceFiles), JSON_UNESCAPED_SLASHES);
    } elseif (is_string($evidenceFiles) && trim($evidenceFiles) !== '') {
        $photosJson = trim($evidenceFiles);
    }

    $pdo->prepare("
        INSERT INTO farm_visits
            (farm_id, task_id, agent_id, visit_latitude, visit_longitude, distance_from_submitted_location_m, client_visit_id, sync_source, photos, notes, result, visited_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ")->execute([
        (int) $task['farm_id'],
        $taskId,
