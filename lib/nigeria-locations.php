...
function nigeria_lga_map(): array
{
    return [
        ...
        'AD' => ['Demsa', 'Fufore', 'Ganye', 'Gayuk', 'Gombi', 'Grie', 'Hong', 'Jada', 'Lamurde', 'Madagali', 'Maiha', 'Mayo Belwa', 'Michika', 'Mubi North', 'Mubi South', 'Numan', 'Shelleng', 'Song', 'Toungo', 'Yola North', 'Yola South'],
        ...
        'BA' => ['Alkaleri', 'Bauchi', 'Bogoro', 'Damban', 'Darazo', 'Dass', 'Gamawa', 'Ganjuwa', 'Giade', 'Itas/Gadau', 'Jamaare', 'Katagum', 'Kirfi', 'Misau', 'Ningi', 'Shira', 'Tafawa Balewa', 'Toro', 'Warji', 'Zaki'],
        ...
        'OS' => ['Atakunmosa East', 'Atakunmosa West', 'Aiyedaade', 'Aiyedire', 'Boluwaduro', 'Boripe', 'Ede North', 'Ede South', 'Egbedore', 'Ejigbo', 'Ife Central', 'Ife East', 'Ife North', 'Ife South', 'Ifedayo', 'Ifelodun', 'Ila', 'Ilesa East', 'Ilesa West', 'Irepodun', 'Irewole', 'Isokan', 'Iwo', 'Obokun', 'Odo Otin', 'Ola Oluwa', 'Olorunda', 'Oriade', 'Orolu', 'Osogbo'],
...
function nigeria_state_code_from_name(string $state): string
{
    ...
    $map = [
        ...
        'gombe' => 'GO', 'imo' => 'IM', 'jigawa' => 'JI', 'kaduna' => 'KD', 'kano' => 'KN', 'katsina' => 'KT',
        'kebbi' => 'KE', 'kogi' => 'KO', 'kwara' => 'KW', 'lagos' => 'LA', 'nasarawa' => 'NA', 'nassarawa' => 'NA',
        'niger' => 'NI', 'ogun' => 'OG', 'ondo' => 'ON', 'osun' => 'OS', 'oyo' => 'OY', 'plateau' => 'PL',
        'rivers' => 'RI', 'sokoto' => 'SO', 'taraba' => 'TA', 'yobe' => 'YO', 'zamfara' => 'ZA',
    ];

    return $map[$normalized] ?? strtoupper($state);
}

function nigeria_lgas_for_state(string $state): array
{
    $code = nigeria_state_code_from_name($state);
    return nigeria_lga_map()[$code] ?? [];
}

function nigeria_ensure_lgas_for_state(PDO $pdo, int $stateId, string $stateName, ?string $stateCode = null): array
{
    ...
    $lgas = $stateCode ? (nigeria_lga_map()[strtoupper($stateCode)] ?? []) : nigeria_lgas_for_state($stateName);
    if (!$lgas) {
        return [];
    }

    $insert = $pdo->prepare("INSERT IGNORE INTO nigeria_lgas (lga_name, state_id) VALUES (?, ?)");
    foreach ($lgas as $lga) {
        $insert->execute([$lga, $stateId]);
    }
